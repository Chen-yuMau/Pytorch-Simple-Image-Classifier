...
add_timestats = [7.753340653558185e-09, -1.5546240846127425e-05, 0.015382630278059401, 1.0269246451454956]
def func(x, a, b, c, d):
...
##################################################################################################
	...
	while c<=cend:
		...
		av_epoch_time_list.append(epochtaverage)
		...
		for a in Tacc:
			...
			for b in a:
				asum+=b
			avacc.append(asum/av)
		xcord = list(range(1,epochs+1))
		f.write(f'{c},{ffaveragesum},{epochtaverage}\n')
	##################################################################################################
		color = c - cstart
		...
		plt.plot(xcord, avacc, str(realcolor) )
	##################################################################################################	
	ts = time.strftime('%H:%M:%S', time.gmtime(est))
	print(f"Estimated time: {ts}")
	plt.show()
	plt.plot(cx, cy)
	plt.show()
	plt.plot(cx, av_epoch_time_list)
	plt.show()
